#if !defined(MY_DEFERRED_LIGHTING)
#define MY_DEFERRED_LIGHTING
...
float GetShadowMaskAttenuation(float2 uv)
{
    float attenuation = 1;
        ...
        float4 mask = tex2D(_CameraGBufferTexture3, uv);
...
UnityLight CreateLight(float2 uv, float3 worldPos, float viewZ)
{
    ...
    if(shadowed)
    {
        ...
        #if defined(UNITY_FAST_COHERENT_DYNAMIC_BRANCHING) && defined(SHADOWS_SOFT) // optimization
            #if !defined(SHADOWS_MASK)
                UNITY_BRANCH
                if(shadowFade > 0.99)
                {
                    shadowAttenuation = 1;
